...
static bool test_regex_func(mongory_memory_pool *pool, mongory_value *pattern,
                            mongory_value *value) {
  (void)pool;
  (void)pattern;
  (void)value;
...
static mongory_matcher *test_matcher_build_func(mongory_memory_pool *pool,
...
void test_mongory_matcher_register(void) {
  ...
  mongory_matcher_build_func retrieved =
      mongory_matcher_build_func_get("$test");
...
void test_mongory_matcher_build_func_get(void) {
  ...
  TEST_ASSERT_EQUAL(mongory_matcher_in_new,
                    mongory_matcher_build_func_get("$in"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$nin"));
  TEST_ASSERT_EQUAL(mongory_matcher_not_in_new,
                    mongory_matcher_build_func_get("$nin"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$eq"));
  TEST_ASSERT_EQUAL(mongory_matcher_equal_new,
                    mongory_matcher_build_func_get("$eq"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$ne"));
  TEST_ASSERT_EQUAL(mongory_matcher_not_equal_new,
                    mongory_matcher_build_func_get("$ne"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$gt"));
  TEST_ASSERT_EQUAL(mongory_matcher_greater_than_new,
                    mongory_matcher_build_func_get("$gt"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$gte"));
  TEST_ASSERT_EQUAL(mongory_matcher_greater_than_or_equal_new,
                    mongory_matcher_build_func_get("$gte"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$lt"));
  TEST_ASSERT_EQUAL(mongory_matcher_less_than_new,
                    mongory_matcher_build_func_get("$lt"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$lte"));
  TEST_ASSERT_EQUAL(mongory_matcher_less_than_or_equal_new,
                    mongory_matcher_build_func_get("$lte"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$exists"));
  TEST_ASSERT_EQUAL(mongory_matcher_exists_new,
                    mongory_matcher_build_func_get("$exists"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$present"));
  TEST_ASSERT_EQUAL(mongory_matcher_present_new,
                    mongory_matcher_build_func_get("$present"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$regex"));
  TEST_ASSERT_EQUAL(mongory_matcher_regex_new,
                    mongory_matcher_build_func_get("$regex"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$and"));
  TEST_ASSERT_EQUAL(mongory_matcher_and_new,
                    mongory_matcher_build_func_get("$and"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$or"));
  TEST_ASSERT_EQUAL(mongory_matcher_or_new,
                    mongory_matcher_build_func_get("$or"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$elemMatch"));
  TEST_ASSERT_EQUAL(mongory_matcher_elem_match_new,
                    mongory_matcher_build_func_get("$elemMatch"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$every"));
  TEST_ASSERT_EQUAL(mongory_matcher_every_new,
                    mongory_matcher_build_func_get("$every"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$not"));
  TEST_ASSERT_EQUAL(mongory_matcher_not_new,
                    mongory_matcher_build_func_get("$not"));
  TEST_ASSERT_NOT_NULL(mongory_matcher_build_func_get("$size"));
  TEST_ASSERT_EQUAL(mongory_matcher_size_new,
                    mongory_matcher_build_func_get("$size"));
