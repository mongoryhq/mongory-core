...
static inline mongory_value* mongory_matcher_array_record_parse_table(mongory_value *condition) {
  ...
  mongory_matcher_array_record_parse_table_context context = { parsed_table, elem_match_table };
  mongory_table *table = condition->data.t;
  table->each(table, &context, mongory_matcher_array_record_parse_table_foreach);
  if (elem_match_table->count > 0) {
    parsed_table->set(parsed_table, "$elemMatch", mongory_value_wrap_t(condition->pool, elem_match_table));
  }
  return mongory_value_wrap_t(condition->pool, parsed_table);
}

static inline mongory_matcher* mongory_matcher_array_record_elem_match_regex_new(mongory_memory_pool *pool, mongory_value *condition) {
  mongory_table *table = mongory_table_new(pool);
  table->set(table, "$regex", condition);
  return mongory_matcher_elem_match_new(pool, mongory_value_wrap_t(pool, table));
}

static inline mongory_matcher* mongory_matcher_array_record_elem_match_equal_new(mongory_memory_pool *pool, mongory_value *condition) {
  mongory_table *table = mongory_table_new(pool);
  table->set(table, "$eq", condition);
  return mongory_matcher_elem_match_new(pool, mongory_value_wrap_t(pool, table));
}

static inline mongory_matcher* mongory_matcher_array_record_left_delegate(mongory_memory_pool *pool, mongory_value *condition) {
  switch (condition->type) {
    case MONGORY_TYPE_TABLE:
      return mongory_matcher_table_cond_new(pool, mongory_matcher_array_record_parse_table(condition));
...
static inline mongory_matcher* mongory_matcher_array_record_right_delegate(mongory_memory_pool *pool, mongory_value *condition) {
  switch (condition->type) {
